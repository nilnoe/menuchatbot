...
> 测试从 175 增至 189。实测反馈后补充修复：快捷按钮加大到 22pt 且选中行常显、
> 修复 ScrollView 视口测量（background GeometryReader 实测 0×0）让空状态
> 真正居中、对话列改为随主区等比缩放（见第二节）。
>
> **已解决（2026-08-01）**：侧栏会话行快捷按钮（取消置顶 / 重命名 / 删除）
> 真机点击无响应——根因是 macOS 上 `.buttonStyle(.borderless)` 按钮在
> ScrollView + LazyVStack 中点击无效（`plain` / `bordered` 正常）。2026-07-31
> 第三轮修复时误把 plain 改为 borderless 反而引入了问题；已改回 plain 并在
> 真实应用内用进程内 Accessibility / AppKit 事件注入验证 action 触发、置顶
> 状态翻转（见第二节「0.3 实测反馈修复」）。
>
> **第二处已解决（2026-08-01，实测反馈）**：真机复测发现「置顶状态下
> 无法取消置顶 / 重命名失效 / 选中态卡在第一个置顶会话」。根因是侧栏列表
> 的「外层 ForEach(分组) + 内层 ForEach(会话)」嵌套结构——会话在置顶 /
> 时间分组之间移动时，LazyVStack 保留旧位置的过期行快照（按钮 help、
> 闭包捕获的 isPinned / 标题都是移动前的值），导致置顶后的操作全部基于
> 过期数据。已改为单一扁平 ForEach（`SidebarItem`：组头 + 会话行混排），
> 并让置顶 / 重命名闭包从 store 读当前状态而非捕获快照；进程内 AX 注入
> 验证：取消置顶、置顶中重命名、多置顶会话切换选中态均正常。
>
