...
> 全部文档索引见 [docs/README.md](docs/README.md)（文档地图）。

> **当前状态（2026-08-01）**：测试 191 全绿；分层架构重构
> （refactor/architecture）与测试模块化 Phase A（refactor/test-modularization）
> 已完成；侧栏快捷按钮点击无响应、置顶后快捷操作失效两个实测问题已解决
> （根因与验证方法见 [docs/PITFALLS.md](docs/PITFALLS.md)）。
>
> **历史进展存档**（逐项细节见 [CHANGELOG.md](CHANGELOG.md)）：
> 0.2.x 性能里程碑（流式增量刷新 / Markdown 节流缓存 / SQLite 迁移）；
> 0.3 功能（自定义供应商、Token 用量、会话置顶、自定义窗口大小）；
> 0.3 UI 收尾与实测修复（侧栏行信息重叠 / 空状态居中 / 窗口档位）。
>
> **下一步方向**：① 键盘快捷键（⌘N / ⌘1~9 / Esc 收面板）与全局热键
> （如 ⌥Space）按需求暂不做；② Beta 0.4 候选：归档 / 标签、多语言、
> 无障碍（见下）；③ 1.0 路径：Developer ID 签名公证 + dmg + Sparkle 更新。
>
> **2026-08-01 规划定稿**：Rust 核心与 AI 能力（资料库 RAG / MCP 工具宿主 /
> 长时推演）按实现难度分层见第七节；纯文档规划，未动代码。
...
## 七、Rust 核心与 AI 能力规划（2026-08-01 定稿）

> 方向已定稿：ADR-0004（Rust 集成方案 A）、ADR-0005（资料库 RAG）、
> ADR-0006（MCP 工具宿主）、ADR-0007（长时推演）；
> 详细设计见 [docs/DESIGN_RUST_CORE.md](docs/DESIGN_RUST_CORE.md)。
