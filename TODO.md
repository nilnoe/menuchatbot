...
>
> **当前规划（0.3 UI 收尾）**：侧栏操作按钮与 token 用量重叠、空状态 logo
> 不居中、自定义窗口大小三项列入下方待办（含实现难度评估）；本轮只规划不开发。
>
...
- [ ] **侧栏行操作区与信息重叠**：hover 快捷按钮（置顶 / 编辑 / 删除）会盖住
  右侧 token 用量文本——预留位 44pt 小于三按钮实际宽度（约 66pt），且 176pt
  侧栏内标题、时间、消息条数、tokens 挤在同一行。
  - 候选方案：① 预留位与按钮组实际宽度对齐；② 把 tokens 用量移到气泡行或
    独立子行，侧栏只保留「条数」；③ 加宽侧栏 / 调整 sidebar 与 main 区比例
    （`ContentView` 里 176pt 常量，可入 DesignTokens）。
  - 实现难度：**中**——涉及布局度量与可能的宽度常量调整，需用
    `BubbleLayoutTests` 同类布局回归测试护航，避免凭直觉调宽度。

- [ ] **空状态 logo 居中**：当前 `emptyState` 在消息区 ScrollView 的 `else`
  分支，仅 `.padding(.top, 80)` 粗略下移，窗口高度变化时不真正居中。
  - **倒排列表必要性（先查证，勿擅改）**：消息区采用倒置列表（容器与每行
    各旋转 180° 双重旋转）是为了绕开「LazyVStack 程序化滚动到未物化区域 →
    空白」的已知缺陷（Apple 论坛 741406 同款，见 CHANGELOG 踩坑记录 #4）；
    试过的错误方案（底部锚点移出列表 / 末几条移出列表杀懒加载）均被否决，
    该取舍不能取消。
  - 空状态与消息区共用同一 ScrollView：若并入倒置容器需再双重旋转恢复文字
    方向，并与贴底判断联动；若保留 `else` 分支则用视口高度几何居中即可。
  - 实现难度：**中低**（保留 else 分支、用视口高度居中较简单；并入倒置容器
    需处理旋转与贴底逻辑，复杂度上升）。

- [ ] **自定义窗口大小**：设置页新增「窗口大小」项，用户手动设置（如
  占可见区域比例 / 预设档位），UserDefaults 持久化，每次启动按设置生效，
  不再被 autosave 恢复的旧 frame 覆盖。
  - 现状：`PanelController` 默认 93% 铺满硬编码 + `setFrameUsingName` /
    autosave 恢复用户拖拽尺寸，无设置入口。
  - 需明确：设置值与 autosave 的优先级（设置生效时是否忽略/重置 autosave），
    设置变更后是否立即调整当前窗口或仅下次启动生效。
  - 实现难度：**低-中**（`PanelSizing.defaultFrame` 参数化 + SettingsStore
    持久化；边界在 autosave 优先级与实时生效语义）。
