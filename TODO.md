...
>
> **已知未解决（2026-07-31 收工）**：侧栏会话行快捷按钮（取消置顶 / 重命名 /
> 删除）在真机上点击仍无响应。已尝试并排除：共享 hover 竞态（改每行自持）、
> ZStack 重叠命中（改并排 HStack）、plain 样式（改 borderless）、alert 弹窗
> （改行内编辑）。测试环境无法用合成鼠标事件驱动 SwiftUI 按钮，缺少真机交互
> 验证手段；下一步需在真机上用 Accessibility / Instruments 确认事件是否进入
> SwiftUI 命中链。
>
...
- [ ] **侧栏快捷按钮点击无响应（未解决）**：见顶部「已知未解决」；涉及的
  代码结构已多次调整（行自持 hover、并排 HStack、borderless 样式、行内
  重命名），测试 190 全绿但均无法覆盖真机鼠标事件链路，待真机定位。
