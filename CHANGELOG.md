...
- **实测反馈修复（第二轮）**
  ...
  - **侧栏按钮点击没反应（第三轮）**：根因是行结构——整行 Button 与叠在其上
    ...
    （无重叠命中），快捷按钮样式由 plain 改为 borderless（与窗口内其他图标
    按钮一致）；选中行 / hover 行均有完整可点击区域。
...
- **侧栏会话行快捷按钮点击无响应**：取消置顶 / 重命名 / 删除按钮在真机上
  点击仍无反应（2026-07-31 收工时仍未解决）。已尝试并排除：行自持 hover
  （消除共享状态竞态）、行内容与快捷按钮并排 HStack（消除 ZStack 重叠命中）、
  快捷按钮 borderless 样式、重命名改行内编辑。测试环境无法用合成鼠标事件
  驱动 SwiftUI 按钮，缺真机交互验证手段；下一步在真机上用 Accessibility /
  Instruments 确认事件是否进入 SwiftUI 命中链。
