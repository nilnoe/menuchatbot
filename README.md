...
>
> 当前版本：**Beta 0.3.3** · 纯 Swift / SwiftUI 实现，无 Web 框架、无 WebView、无运行时服务进程。
...
| 只读文件工具 | 开启后模型可通过结构化 JSON 参数读取已授权资料库目录内的
  文本文件（相对路径 + 行号范围，扩展名白名单 / 大小与行数上限），无 shell、
  无写权限，越权路径一律拒绝 |
| API Key 安全存储 | 存于 macOS 钥匙串（Keychain），不上传、不写入代码 |
| 安全审计 | 设置页「安全审计」：配置 / 权限 / 工具 / 存储事件结构化记录
  （独立 audit.sqlite，追加式，90 天 / 50MB 保留），支持过滤与导出
  （不含密钥与消息全文）；工具调用每次执行 start/end 成对可对账 |
...
- **审计记录**：独立 `audit.sqlite`（同目录），记录配置变更、权限通过 /
  拒绝、工具执行、迁移 / 导入导出等安全事件；默认保留 90 天或 50MB，
  设置页可查看与导出
